...
def Deep_Q(environment, epsilon=0.1, alpha=0.1, gamma=0.99, epoch_number=8000, pretrained_model: None | keras.Sequential = None):
    ...
    if pretrained_model is None:
        ...
        model.add(layers.Dense(24, input_shape=(
            environment_space_length,), activation='relu'))
    ...
    def update(state_index, action_index, next_state, reward: float = 0, done=False):
        target = model.predict(np.array([state_index]))[0]
        if done:
            target[action_index] = reward
        else:
            target[action_index] = reward + gamma * \
                np.max(model.predict(np.array([next_state])))
        ...
        while not stop:
            ...
            if epoch % 100 == 0:  # print log every 100 episode
                print("episode: {}/{}, moves: {}, score: {}"
                      .format(epoch, epoch_number, steps, score))
    return model
